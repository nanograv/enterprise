...
def psr():
    return Pulsar(datadir + "/B1855+09_NANOGrav_9yv1.gls.par", datadir + "/B1855+09_NANOGrav_9yv1.tim")
...
def ipsr():
    # IPTA-like pulsar
    if t2 is None:
        pytest.skip("T2 timing model so not supported by PINT")
    return Pulsar(datadir + "/1713.Sep.T2.par", datadir + "/1713.Sep.T2.tim")
...
def test_ecorr_ipta(ipsr, method):
    ...
    # get EFAC Nvec
    nvec0 = efacs[0] ** 2 * pulsar.toaerrs**2

    # get the basis
    flags = ["ASP-L", "ASP-S", "GASP-8", "GASP-L", "GUPPI-8", "GUPPI-L", "PUPPI-L", "PUPPI-S"]
    bflags = pulsar.backend_flags
    Umats = []
    for flag in np.unique(bflags):
        if flag in flags:
            mask = bflags == flag
            Umats.append(utils.create_quantization_matrix(pulsar.toas[mask], nmin=2)[0])
    nepoch = sum(U.shape[1] for U in Umats)
    U = np.zeros((len(pulsar.toas), nepoch))
    jvec = np.zeros(nepoch)
    netot, ct = 0, 0
    for flag in np.unique(bflags):
        if flag in flags:
    ...
    msg = "EFAC/ECORR {} logdet incorrect.".format(method)
    N = m.get_ndiag(params)
    assert np.allclose(N.solve(pulsar.residuals, logdet=True)[1], wd.logdet(), rtol=1e-8), msg

    msg = "EFAC/ECORR {} D1 solve incorrect.".format(method)
    assert np.allclose(N.solve(pulsar.residuals), wd.solve(pulsar.residuals), rtol=1e-8), msg

    msg = "EFAC/ECORR {} 1D1 solve incorrect.".format(method)
    assert np.allclose(
        N.solve(pulsar.residuals, left_array=pulsar.residuals),
        np.dot(pulsar.residuals, wd.solve(pulsar.residuals)),
        rtol=1e-8,
    ), msg

    msg = "EFAC/ECORR {} 2D1 solve incorrect.".format(method)
    T = m.get_basis()
    assert np.allclose(N.solve(pulsar.residuals, left_array=T), np.dot(T.T, wd.solve(pulsar.residuals)), rtol=1e-8), msg
